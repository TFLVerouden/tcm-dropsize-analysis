import os
"""
Generates all measurements within one series in a plot
"""
...
for file in matching_files:
    ...
    if mask and i<6:
        ...
        mode= "ln"
        x,pdf,per,n_peaks = fitting(n_pdf,bin_centers,mode=mode)
        # x_h = np.empty(len(bin_centers) * 3 - 3)
        # y_h = np.empty(len(bin_centers) * 3 - 3)

        # # Fill with pattern: [x_i, x_{i+1}, nan]
        # x_h[::3] = bin_centers[:-1]
        # x_h[1::3] = bin_centers[1:]
        # x_h[2::3] = np.nan

        # # Corresponding y-values (flat for each segment)
        # y_h[::3] = n_percentages[:-1]
        # y_h[1::3] = n_percentages[:-1]
        # y_h[2::3] = np.nan

        # plt.plot(x_h, y_h, color=colors[i], lw=2)
        plt.step(bin_centers,n_percentages,where="mid",color= colors[i])
        plt.xscale('log')
        ...
        plt.tight_layout()
        #plt.legend()
        plt.savefig(fr"C:\Users\sikke\Documents\universiteit\Master\Thesis\presentation\measurementseries0dot03percent_{i}.svg")
        # plt.step(bin_centers,n_percentages,where="mid",color=colors[i])
        ...
        # x = x[mask]
        # y = y[mask]
        #plt.fill_between(x, 0, y, step='mid', color=colors[i], alpha=0.2)
